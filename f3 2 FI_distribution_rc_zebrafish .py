...
for path in ['_P2M','_P2S','_P2X']:
    average_path = np.load(f'../path/RC_path/zebrafish{path}_rc.npy')
    for k_nei1, K1, L1 in product(k_nei_values, K_values, L_values):
        ...
        result_path = 'FI_distribution_rc/'+data_name+f'{path}/'
        encoder_path = '../encoder/'+data_name+' '+str([k_nei,K,L])+'/'
        ...
        for k in range(len(cell_arr)):
            ...
            x00=torch.tensor(mean_cell@adata.varm['PCs'],dtype=torch.float32, requires_grad=True)
            z00=model(x00)
